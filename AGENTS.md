...
Legacy autopost lives in [`Scheduler`](src/Scheduler.php) when `migration_mode = legacy`. New pipeline work: hooks → enqueue → GraphQL on flush, not sync in the save hook. CMS-specific conversion of legacy autopost templates (macros, conditions, UTM/VK, delay) lives in [`src/Migration/`](src/Migration/) and is pushed as GraphQL `MigratedPipelineConfigInput` via [`MigrationService`](src/MigrationService.php) — the backend does not parse WP dialect. Same post type / conditions / delay / media are clustered into one pipeline ([`TemplateClusterer`](src/Migration/TemplateClusterer.php) + [`PayloadBuilder::from_cluster`](src/Migration/PayloadBuilder.php)): union of accounts; per-network text/UTM via `socialTypeOverrides` when social types do not overlap. Same network + different content stays as separate pipelines.
...
- **GraphQL:** POST [`Env::graphql_api_uri()`](src/Env.php) (`/api/graphql`); HMAC path is [`Env::graphql_signing_path()`](src/Env.php) (`/graphql` after nginx strips `/api`). HMAC query `pluginWpPostsBySourceItem` lists PP posts for the current WP item (`Api::list_posts_by_cms_source`; v2 provenance ∪ legacy `extra.wp_post_id`); `pluginPipelinesForPublish(sourcePath)` is the publish-modal pipeline list; `pluginWpPostsBySourceItemsBatch(sourceItemIds, siteDomain)` is the edit.php column batch. Result rows include `errorMessage`. `plugin_id` is from auth, not input. `pluginPipelineEventIngest` payload is `[SourceFieldInput!]!`, not raw JSON ([`PushEventService::payload_to_source_fields`](src/PushEventService.php)).
...
- **Pipeline events:** enqueue on save; ingest on flush. `idempotencyKey` required. `pipeline.not_found` → drop id via `Settings::remove_pipeline_id` and delete the row (no retry). `_pp_prev_fields` updates only after successful ingest. `source_item_id` is `{post_type}:{post_id}`. Taxonomy fields in wire/pipeline payloads are `[{id, name}, …]` (`id` for `includes_*`, `name` for Liquid text); legacy macros still use names via `Taxonomies::get_field_value`. `SelectionFilterQuery` is fail-closed (unsupported AST → 422 `unsupported_selection_filter`).
...
Smoke tests (no live WPDB unless noted): `bin/test-wp*.php`. PHP/WP version matrix: `make compat` (`compat-php` = `php:7.4-cli`/`8.5-cli` lint + smokes; `compat-wp` = ephemeral [`docker-compat/`](docker-compat/) cells). `docker-compat/` is not the daily bind-mount stack and not upgrade ZIPs — no host port, `down -v` per cell. Daily docker: `make docker-up` (bind-mounts this repo). `wp-content/uploads` is named volume `wp_data`; Apache is `www-data` (uid 33) while `docker compose exec` is root — do not create files under `uploads/` as root (`docker/ensure-uploads-writable.sh` re-chowns that tree on start). WP-Cron in compose is service `wp-cron` (`DISABLE_WP_CRON`); do not rely on pseudo-cron to `wp-pp.l2.vettich.ru`. Upgrade ZIPs: [`docker-upgrade/`](docker-upgrade/) + [`meta/local-dev/checklist-plugin-upgrade.md`](../meta/local-dev/checklist-plugin-upgrade.md). Pipeline e2e notes: [`meta/local-dev/wordpress-pipeline-testing.md`](../meta/local-dev/wordpress-pipeline-testing.md).
