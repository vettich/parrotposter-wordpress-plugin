...
- **Cron:** hook `OutboundTaskWorker::CRON_HOOK` (`parrotposter_outbound_lease_tick`), fixed base interval [`OutboundTaskWorker::BASE_INTERVAL_SEC`](src/OutboundTaskWorker.php) (10 min, SPEC-002-03 §10.1) via a dedicated `cron_schedules` entry. A single easily-swappable constant — WP-10 makes the interval adaptive (`primaryHealth` / `recommendedPollIntervalS` / backoff+jitter); not built yet. Scheduled in `Install::install()` and `PP::activation()`, cleared in `PP::deactivation()`, matching the `parrotposter_retry_local_queue` pattern.
...
- **Dispatch is a WP-09 stub:** signature verification (Ed25519 vs `outbound_task_signing_public_key`) and per-`type` dispatch (`fetch_next`/`fetch_batch`/`rotate_secrets`/`ping`) are explicitly out of scope for WP-08. `dispatch_task()` always reports `skipped` unless a later change hooks the `parrotposter_outbound_task_dispatch` filter (`(?array $outcome, array $row) → array{status, result, error_code}|null`) — WP-09's intended extension seam, so it never needs to touch `OutboundTaskWorker`/`OutboundTaskQueue` internals.
- **Test:** [`bin/test-wp08-outbound-worker.php`](bin/test-wp08-outbound-worker.php) — pure-logic smoke tests (idempotent re-lease, expired short-circuit, time-budget enforcement) with an injected fake clock, no WPDB.
